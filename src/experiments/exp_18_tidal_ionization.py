...
Provenance.
  ...
  This file is reproduced verbatim from Paper~I of the A=1 Discrete
  Causal Lattice series (doi:10.5281/zenodo.20078529).  In Paper~I's
  own repository it lives at
  src/experiments/exp_18_tidal_ionization.py and was the planned
  test for the gradient-ionization argument of
  notes/plasma_as_gravitational_ionization.md.  It is reproduced
  in this Paper~III repo so that the paper's audit table can cite
  it as the load-bearing numerical evidence without depending on
  the Paper~I repository at runtime.

  Engine dependency.  This script imports OctahedralLattice,
  CausalSession, and enforce_unity_spinor from dcl_core.core --
  the continuous-amplitude submodule of the shared dcl_core
  package, which is a verbatim port of Paper~I's src/core/.  The
  dependency is declared in virtual-env-requirements.txt as
  `dcl_core @ git+https://github.com/JackDMenendez/dcl-core@main`;
  `setup.cmd` / `setup.sh` install it into the venv automatically.
